...
import pygame
import random
from config import (BLACK, WHITE, SCREEN_WIDTH, SCREEN_HEIGHT, BALL_RADIUS,BALL_BASE_SPEED_X, BALL_BASE_SPEED_Y, BALL_SPEED_INCREASE_FACTOR, BALL_MAX_SPEED_Y_FACTOR)
    ...
    def update(self, player_paddle, opponent_paddle):
        ...
        for paddle in paddles:
            if self.rect.colliderect(paddle.rect):
                ...
                moving_towards_player_paddle = self.velocity_x < 0 # Ball moving left
                moving_towards_opponent_paddle = self.velocity_x > 0 # Ball moving right
                ...
                if (is_player_paddle and moving_towards_player_paddle) or \
                   (not is_player_paddle and moving_towards_opponent_paddle):                
                    ...
                    if not is_player_paddle: # Opponent paddle hit
                        self.velocity_x = -abs(self.velocity_x) 
                    else: # Player paddle hit
                        self.velocity_x = abs(self.velocity_x)
